...
class VKOGA_Structured(BaseEstimator):
...
#        X, y = check_X_y(X, y, multi_output=True)
        ...
        for n in range(self.max_iter):
            ...
            Vx[notIndI, n] = self.kernel.eval(sublist(X, notIndI), X[self.indI_[n]])[:, 0] \
        ...
        self.ctrs_ = list(itemgetter(*self.indI_)(X))
...
def sublist(L, idx):
    ''' Rerurns the sublist of the list L with indices idx'''
    sub = itemgetter(*idx)(L)
    if len(idx) > 1:
        sub = list(sub)
    else:
        sub = [sub]   
